...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    const [monthlyCollection] = await pool.query('SELECT COALESCE(SUM(paid_amount), 0) as total FROM fees WHERE MONTH(paid_date) = ? AND YEAR(paid_date) = ? AND status = "Paid"', [currentMonth, currentYear]);
