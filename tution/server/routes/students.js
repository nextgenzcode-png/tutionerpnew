...
router.get('/:id/profile', authMiddleware, async (req, res) => {
  try {
    ...
    const [monthlyTrend] = await pool.query(`
      SELECT MONTHNAME(sub.exam_date) as month, MONTH(sub.exam_date) as month_num, YEAR(sub.exam_date) as year,
             AVG((sm.marks_obtained / sub.max_marks) * 100) as avg_percentage
      FROM subject_marks sm
      JOIN subjects sub ON sm.subject_id = sub.id
      WHERE sm.student_id = ?
      GROUP BY YEAR(sub.exam_date), MONTH(sub.exam_date), MONTHNAME(sub.exam_date)
      ORDER BY YEAR(sub.exam_date) ASC, MONTH(sub.exam_date) ASC
    `, [studentId]);
